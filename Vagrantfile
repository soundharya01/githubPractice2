...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", env: {"IP_NW" => IP_NW, "IP_START" => IP_START}, inline: <<-SHELL
      apt-get update -y
      echo "$IP_NW$((IP_START))  master-node  master-node.nrg.io" >> /etc/hosts
      echo "$IP_NW$((IP_START+1))  worker-node01  worker-node01.nrg.io" >> /etc/hosts
      echo "$IP_NW$((IP_START+2))  worker-node02  worker-node02.nrg.io" >> /etc/hosts
      echo "$IP_NW$((IP_START+3))  worker-node03  worker-node03.nrg.io" >> /etc/hosts
  SHELL
  ...
  config.vm.define "master" do |master|
    ...
    master.vm.hostname = "master-node.nrg.io"
  ...
  (1..NUM_WORKER_NODES).each do |i|
    ...
    config.ssh.username = "vagrant"
    config.ssh.password = "vagrant"
      node.vm.hostname = "worker-node0#{i}.nrg.io"
